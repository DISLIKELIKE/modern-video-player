...
2. WHEN configuration keys are missing, the system SHALL apply documented defaults (`log_dir=logs`, `max_file_size_mb=10`, `max_files=5`, `log_level=info`) and SHALL expose them via `docs/design/LOGGING.md`.
...
1. WHEN the feature ships, `docs/design/LOGGING.md` SHALL describe the Quill configuration workflow, default/fallback behavior, environment overrides, and troubleshooting steps for permission errors.
2. WHEN documentation is updated, `docs/records/CHANGELOG.md` SHALL add an entry summarizing the enterprise logging feature, and `docs/records/VERSION.md` SHALL update the Quill dependency status plus new configuration artifacts.

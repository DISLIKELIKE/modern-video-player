...
- [ ] 1.1 创建线程安全任务队列（TaskQueue）
  - [ ] 1.1.1 设计无锁队列（LockFreeQueue）或互斥锁队列
  - [ ] 1.1.2 实现 enqueue/dequeue 操作
  - [ ] 1.1.3 添加线程安全断言

- [ ] 1.2 创建帧缓冲池（FramePool）
  - [ ] 1.2.1 实现预分配的 AVFrame 池
  - [ ] 1.2.2 实现帧获取/归还机制
  - [ ] 1.2.3 实现池大小可配置

- [ ] 1.3 创建解码线程基类
  - [ ] 1.3.1 设计 DecoderThread 接口
  - [ ] 1.3.2 实现线程启动/停止/暂停控制
...
- [ ] 1. 定义滤镜基类（FilterBase）
  - [ ] 1.1 定义 IFilter 接口
  - [ ] 1.2 定义 FilterType 枚举（Video, Audio）
  - [ ] 1.3 定义输入/输出pin接口

- [ ] 2. 实现视频滤镜链（VideoFilterChain）
  - [ ] 2.1 创建 VideoFilterChain 管理类
  - [ ] 2.2 实现 addFilter/removeFilter 方法
  - [ ] 2.3 实现滤镜顺序管理
  - [ ] 2.4 实现链式处理方法

- [ ] 3. 实现音频滤镜链（AudioFilterChain）
  - [ ] 3.1 创建 AudioFilterChain 管理类
  - [ ] 3.2 实现音频帧处理接口
...
- [ ] 1. 抽象渲染器接口（IVideoRenderer）
  - [ ] 1.1 定义 IRenderer 接口
  - [ ] 1.2 定义支持的渲染类型（Software, D3D11, OpenGL）
  - [ ] 1.3 定义初始化和渲染方法
...
- [ ] 3. 实现 D3D11 渲染器
  - [ ] 3.1 创建 D3D11Renderer 类
  - [ ] 3.2 实现 Direct3D 设备初始化
  - [ ] 3.3 实现纹理创建和更新
  - [ ] 3.4 实现视频帧显示

- [ ] 4. 实现 OpenGL 渲染器
  - [ ] 4.1 创建 OpenGLRenderer 类
  - [ ] 4.2 实现 OpenGL 上下文初始化
  - 4.3 实现着色器程序
  - [ ] 4.4 实现 YUV 转 RGB 并渲染

- [ ] 5. 渲染器管理器
  - [ ] 5.1 实现 RendererFactory
  - [ ] 5.2 实现渲染器自动选择逻辑
...
- [ ] 1. 字幕解析器基类
  - [ ] 1.1 定义 ISubtitleParser 接口
  - [ ] 1.2 定义 SubtitleItem 结构
...
- [ ] 1. 定义播放列表数据结构
  - [ ] 1.1 定义 PlaylistItem 类
  - [ ] 1.2 实现媒体信息（时长、分辨率、编码）

- [ ] 2. 实现播放列表管理器
  - [ ] 2.1 创建 PlaylistManager 类
  - [ ] 2.2 实现 add/remove/clear 方法
  - [ ] 2.3 实现多种排序方式（名称、时间、时长）
...
- [ ] 1. 解码器接口抽象
  - [ ] 1.1 定义 IDecoder 接口
  - [ ] 1.2 定义 DecoderCapability 结构

- [ ] 2. 解码器工厂
  - [ ] 2.1 创建 DecoderFactory 类
  - [ ] 2.2 实现软解/硬解自动选择
...
- [ ] 2. 实现特殊格式支持
  - [ ] 2.1 添加 MKV 特性支持（章节、轨道）
  - [ ] 2.2 添加 MP4 特性支持（moov 预加载）
...
- [ ] 1. 设置系统
  - [ ] 1.1 定义 SettingsManager 类
  - [ ] 1.2 实现 INI/JSON 配置存储
  - [ ] 1.3 实现设置项热重载

- [ ] 2. 快捷键系统
  - [ ] 2.1 定义 HotkeyManager 类
  - [ ] 2.2 实现按键绑定配置

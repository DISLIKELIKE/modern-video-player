# Cross-platform Vulkan Windows strict-diag-exit-nonzero expected-fail canary local check

Date: 2026-03-27  
Host: Windows workstation

## 1. Scope
- Validate `VK-043` deterministic strict-diag-exit-nonzero expected-fail canary and workflow integration.
...
### Strict-diag-exit-nonzero expected-fail canary
```text
...
## 3. Conclusion
- Windows Vulkan CI now has deterministic coverage for strict availability detail branch `diag-exit-nonzero`.
- Combined canary matrix now covers diagnostics contract fail, playback contract fail, strict PASS, strict unavailable with `compiled-in-false`, strict unavailable with `runtime-unavailable`, strict unavailable with `diag-exit-nonzero`, optional-skip, unsupported-platform fail, selected-renderer semantic fail, backend semantic fail, candidates semantic fail, plan-reason semantic fail, result-not-pass fail, and command-exit-nonzero fail branches.

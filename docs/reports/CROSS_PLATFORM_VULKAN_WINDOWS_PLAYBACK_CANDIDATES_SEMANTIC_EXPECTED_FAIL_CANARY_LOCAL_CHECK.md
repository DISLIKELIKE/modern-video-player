# Cross-platform Vulkan Windows playback-candidates semantic expected-fail canary local check

Date: 2026-03-27  
Host: Windows workstation

## 1. Scope
- Validate `VK-038` deterministic playback-candidates semantic expected-fail canary and workflow integration.
...
### Playback-candidates semantic expected-fail canary
```text
...
## 3. Conclusion
- Windows Vulkan CI now has deterministic coverage for playback-candidates semantic expected-fail branch.
- Combined canary matrix now covers diagnostics contract fail, playback contract fail, strict PASS, strict-unavailable fail, optional-unavailable skip, unsupported-platform fail, selected-renderer semantic fail, backend semantic fail, and candidate-chain semantic fail branches.

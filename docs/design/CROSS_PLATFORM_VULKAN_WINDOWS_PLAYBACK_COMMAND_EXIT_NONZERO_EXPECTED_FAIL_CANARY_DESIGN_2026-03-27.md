# Cross-platform Vulkan Windows playback-command-exit-nonzero expected-fail canary design (2026-03-27)

## 1. Scope
- Add deterministic expected-fail canary for playback `command-exit-nonzero` branch.
...
- Script behavior:
  ...
  - playback output is contract-valid and then exits non-zero intentionally.
...
- Output contract:
  - machine-readable `windows-vulkan-playback-command-exit-nonzero-canary.*`.

## 4. Workflow Integration
- In Windows gate step:
  - run playback-command-exit-nonzero canary after playback-result-not-pass canary.
  - capture `$vulkanPlaybackCommandExitNonzeroCanaryExitCode` and fail-fast on non-zero.
...
- New playback-command-exit-nonzero canary command.

# Cross-platform Vulkan Windows playback-candidates semantic expected-fail canary design (2026-03-27)

## 1. Scope
- Add deterministic expected-fail canary for playback candidate-chain semantic mismatch branch.
...
- Script behavior:
  ...
  - playback output is contract-valid but semantically invalid:
    ...
    - backend is Vulkan
    - startup candidates omit Vulkan
...
- Output contract:
  - machine-readable `windows-vulkan-playback-candidates-semantic-canary.*`.

## 4. Workflow Integration
- In Windows gate step:
  - run playback-candidates semantic canary after playback-backend semantic canary.
  - capture `$vulkanPlaybackCandidatesSemanticCanaryExitCode` and fail-fast on non-zero.
...
- New playback-candidates semantic canary command.

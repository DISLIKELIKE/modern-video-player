...
- 本地媒体打开、解封装、音视频解码与播放。
- 播放控制：播放、暂停、停止、拖拽进度 seek、快捷键 seek、音量、静音、倍速。
- 基础交互：全屏、控制条覆盖层、字幕显示、章节导航、A-B Repeat、截图。
- 字幕：外挂 `SRT` 加载、渲染叠加、播放/暂停/seek 同步、字幕开关。
- 播放列表：多文件参数 / `m3u8`、上一项/下一项、EOF 自动下一项、恢复上次索引。
- 设置：音量、倍速、硬解偏好、恢复播放列表索引、快捷键持久化。
- 快捷键：默认键位、持久化、冲突检测、恢复默认。
...
| 04 音频渲染器 | `SDL` 音频可用，但均衡器/混音/设备选择/时延调节未接入主链 | 部分实现 |
| 05 字幕系统 | 外挂 `SRT` 已接入播放主链并可同步显示，但内嵌字幕/字幕轨切换/ASS 仍缺失 | 部分实现 |
| 06 播放列表 | 多文件与 `m3u8`、上一项/下一项、EOF 自动下一项、恢复索引已可用，但无 UI/历史/循环模式 | 部分实现 |
| 07 解码器管理 | `DecoderFactory` 已接入真实初始化，`D3D11VA` 与软件回退可运行，但可配置性与可观测性仍偏基础 | 部分实现 |
| 08 文件格式支持 | 主力容器/编码矩阵已有回归结果，但 `1080p60/4K/高码率` 仍未完成稳定性门禁 | 部分实现 |
| 09 流媒体支持 | `HLS/DASH` 解析器仅轻量实现，HTTP 下载器无真实读流/缓冲/ABR | 骨架/未接入 |
| 10 皮肤系统 | 只有主题变量容器，没有资源化主题与组件级样式接入 | 骨架/未接入 |
| 11 快捷键系统 | 默认键位、持久化、冲突检测、恢复默认均已接入，但全量增强键位尚未完成 | 部分实现 |
...
- 尚未完成或未收敛的能力主要集中在：高分辨率稳定性、轨道/时延控制、完整渲染后端、流媒体、插件、皮肤。
...
- `1080p60` 长时稳定播放达标。
- `4K` 可播放并可降级，补齐高码率（`>80Mbps`）性能日志。
- 多音轨切换、字幕轨切换、音轨延迟/字幕延迟调节。

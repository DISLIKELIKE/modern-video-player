...
| 08 文件格式支持 | 主力容器/编码矩阵已有回归结果，`1080p60` 稳定播放与性能日志入口已补齐，但 `4K/高码率` 仍未完成稳定性门禁 | 部分实现 |
...
- 尚未完成或未收敛的能力主要集中在：`4K/高码率` 稳定性、轨道切换、完整渲染后端、流媒体、插件、皮肤。
...
里程碑 2（补“发布门禁”）：
- 完成 `M2 2.2`：`4K`、高码率样本稳定性门禁；`1080p60` 与性能日志验收入口已补齐。

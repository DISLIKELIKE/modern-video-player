...
- `Scheduler` render wait 改为消费策略函数（`isAudioMasterActive` / `isVideoMasterActive` / `shouldApplyClockSync`），同时 `Scheduler::stop()` 新增 self-join 保护。

- `FailSession` 已从预留入口推进到关键失败点实用路径；`handleRuntimeFailure()` 同步补入 stop/session failure 计数。

- diagnostics 新增 stale serial 丢弃计数，明确 `queue generation` 只负责容器边界中断，而旧时间线硬失效仍由 item-level serial 主判定。
### 2026-03-20 更新：PlayerCore 副作用集中化与 runtime failure/recovery policy 收口

- `SchedulerControlSnapshot` 已补入 `clock_source`、`audio_output_initialized`、`audio_master_sync_active` 与 `ended_policy`，scheduler 不再只靠最小态和零散推断消费业务语义。

- `PlayerCore` 已把 start/resume/pause/stop request/stop completion/seek/session release 副作用统一抽到 `apply*SideEffects()`，`play / pause / stop / seek / close / requestDeferredStop / serviceDeferredStop` 入口不再直接堆线程和设备动作。

- `deferred stop` 已并回统一 stopping 路径；runtime fatal 现通过 `FailureRecoveryPolicy + handleRuntimeFailure()` 收口到统一恢复入口，后续可以继续扩覆盖范围而不再复制恢复逻辑。

- 本轮不改外部 `PlaybackState`、UI、copy-back 或 SoftwareSDL，只继续收紧内核状态机与 worker/device 动作的边界。

### 2026-03-20 更新：PlayerCore seek/flush timeline serial 化第二阶段



- `PlayerCore` 已引入 `timeline_serial / pending_seek_serial`，seek 不再只靠 flush 和布尔位切换时间线。

- demux packet、decoded `VideoFrame / AudioFrame`、render 和 audio consumer 链路都已接上 serial 硬失效判定。

- `flush` 继续保留，但语义已从“唯一失效机制”降级为“辅助清扫”；真正的旧时间线边界由 serial 提供。

- diagnostics 与专项检查命令已新增 `timeline_serial / pending_seek_serial` 输出，便于后续继续做 EOF/Ended 和 Scheduler 契约升级。





### 2026-03-19 更新：补齐 records / analysis 文档一致性



- `CHANGELOG` 的问题总表已补上 `问题 78` 索引，避免今天这组 software decode 诊断记录在总表里出现 `77 -> 79` 跳号。

- 已回填 `问题 69` 对应的 analysis 文档：`docs/analysis/PLAYERCORE_DAY6_DEFERRED_STOP_AND_RUNTIME_DEBT_FIX.md`。

- 本轮不涉及代码逻辑变更，只收口今天提交后的文档索引和分析沉淀一致性。
...
- 用户要求先做播放器内核状态机重设计第一阶段，不动 serial、不动 copy-back/SoftwareSDL，不改外部 `PlaybackState` 接口。

- 目标是把 UI 播放态和内核会话/流水线态拆开，并把散落在 `PlayerCore` 各入口里的状态改写收口到统一 transition 入口。



### 分析记录

1. 当前 `PlayerCore` 的 `open / play / pause / stop / seek / requestDeferredStop / serviceDeferredStop / onRenderIdle` 都会直接写 `state_`，状态变化没有单一入口。

2. `deferred stop` 之前是独立布尔语义，导致运行态和 deferred stop 旁路态分裂。

3. `Scheduler` 目前只有 `running_ / paused_`，第一阶段不宜过早塞入更多业务语义，应该先由 `PlayerCore` 统一维护会话态、运行态和流水线态。
...
- 新增 `transitionSessionState / transitionRunState / transitionPipelinePhase / publishPlaybackStateFromInternalState`，并加入非法迁移保护和状态迁移日志。

- 将 `eof_reached / pending_seek / deferred_stop_pending` 收回到统一快照管理。

- 把 `open / close / play / pause / stop / seek / requestDeferredStop / serviceDeferredStop / onRenderIdle` 改为只通过 transition helper 变更状态；对外 `PlaybackState` 仍保持兼容。

- 新增分析文档 `docs/analysis/PLAYERCORE_DAY16_STATE_MACHINE_REDESIGN.md`，记录第一阶段边界、Scheduler 处理结论和下一阶段 serial 化建议。
...
- 用户要求继续做播放器内核状态机重设计第二阶段，但明确不动 copy-back、SoftwareSDL、UI 层和外部 `PlaybackState`，只处理 seek/flush 的 timeline serial 化。

- 目标是让 seek/stop/deferred stop 之后的旧 packet、旧视频帧、旧音频帧即使晚到，也必须因为 serial 不匹配而被硬丢弃。



### 分析记录

1. `ThreadSafeQueue` 和 `FrameQueue` 当前都只有 stop/clear/flush 语义，没有 generation/serial；旧时间线数据此前只能靠“被及时清空”而不是“被明确判废”。

2. seek 之前主要依赖 `scheduler pause + stopDemuxThread + flushPipelines + avcodec_flush_buffers() + audio_player_->stop()`，audio consumer 线程本身不带时间线判定，render 路径也没有 serial 防线。

3. 第二阶段最稳妥的落点，是保持 queue 容器通用，改为让每个 packet/frame 自带 serial，并由 `PlayerCore` 集中分配和激活 serial。



### 解决方案

- 新增 `TimelineSerial` 与 `DemuxPacket`，并为 `VideoFrame / AudioFrame` 增加 `serial` 字段。

- 在 `PlayerCore` 内部新增 `timeline_serial / pending_seek_serial` 及统一 helper：`allocateNextTimelineSerial / activateTimelineSerial / setPendingSeekSerial / isAcceptedTimelineSerial`。

- `open` 成功时建立首个 serial；`seek` 先切到 pending serial，再在成功后激活；`stop / requestDeferredStop` 会立即推进 serial，确保旧 worker 晚到时只能产出废数据。

- demux 线程启动时捕获 serial，decode/render/audio consumer 全链路做 stale serial 丢弃；diagnostics 和专项检查命令导出 serial 观测字段。
...
### 问题描述
- `PlayerCore` 的外部入口已经完成状态迁移收口，但线程、设备、队列和时钟副作用仍散落在多个入口函数里。
- `SchedulerControlSnapshot` 还缺 `clock_source`、audio-master 约束和 ended policy，scheduler 仍要自行推断业务语义。
- decode/resample/output 的 fatal 点缺少统一 recovery policy，容易重新长出分散的错误处理。

### 分析记录
1. 状态迁移集中之后，下一层必须继续集中的是 side effects；否则入口函数仍然会成为“状态机 + worker 管理器”的混合体。
2. `deferred stop` 的本质是异步 stop completion，而不是新的业务状态；因此 request/completion side effects 必须共用一套 helper。
3. scheduler 已经拥有 `run_state / pipeline_phase / accepted_timeline_serial`，本轮继续把 `clock_source`、audio-master 和 ended policy 结构化，能避免再加新的零散布尔位。
4. runtime failure 先统一成 `FailureRecoveryPolicy` 后，后续扩大 fatal 点覆盖范围时不需要重新拆恢复路径。

### 处理结果
- 新增 `applyStartPlaybackSideEffects / applyResumePlaybackSideEffects / applyPausePlaybackSideEffects / applyStopRequestSideEffects / applyStopCompletionSideEffects / applySeekSideEffects / applySessionReleaseSideEffects`。
- `requestDeferredStop()` 与 `serviceDeferredStop()` 已复用 stop request/completion side effects，deferred stop 不再维护独立停机逻辑。
- `SchedulerControlSnapshot` 已新增 `clock_source`、`audio_output_initialized`、`audio_master_sync_active`、`ended_policy`，scheduler render wait 逻辑已接入这些字段。
- 新增 `FailureRecoveryPolicy` 与 `handleRuntimeFailure()`，并把视频/音频 decode-resample 链上的关键 fatal 点统一收口到该入口。

### 本地验收结果
- Debug 构建通过，`0 warnings / 0 errors`。
- 未改 UI 层和外部 `PlaybackState`，本轮只收口内核副作用和 runtime recovery 策略。
- 新增分析文档：`docs/analysis/PLAYERCORE_DAY20_SIDE_EFFECTS_AND_RUNTIME_FAILURE_REDESIGN.md`。
...
### 问题描述
- `SchedulerControlSnapshot` 需要继续结构化 ended policy / clock policy / audio-master 约束。
- `FailSession` 需要从统一入口推进到关键运行时错误的实际覆盖。
- queue generation 与 item-level serial 需要更直观的可观测边界。

### 分析记录
1. 若 scheduler 仍依赖布尔拼语义，策略意图会继续分散。
2. 若 `FailSession` 只停留在入口层，遇到不可恢复错误时恢复路径仍不一致。
3. generation 解决的是容器唤醒和边界中断，不替代 serial 的时间线判定。

### 处理结果
- `SchedulerControlSnapshot` 已新增 `clock_policy`、`audio_master_policy`、`audio_buffered_seconds`，并扩展 ended policy。
- `Scheduler` 已改为策略驱动 render wait；`Scheduler::stop()` 增加 self-join 保护。
- `FailSession` 已覆盖关键不可恢复错误点并落到真实路径。
- `DiagnosticsSnapshot` 新增 stale serial drop 计数与 runtime failure 计数，并接入检查命令输出。

### 本地验收结果
- Debug 构建通过，`0 warnings / 0 errors`。
- 新增分析文档：`docs/analysis/PLAYERCORE_DAY21_REMAINING_RISKS_CONVERGENCE.md`。
...
### 问题描述
- 需要补充专用 CLI 回归探针，验证 serial 边界与 FailSession 约束在关键场景下无回归。

### 分析记录
1. 现有 performance/software 检查偏链路健康，不直接覆盖三类状态边界。
2. 非法状态迁移此前只在日志可见，不利于机器 gate。
3. 回归命令需统一 `key=value` 与 `result=PASS/FAIL`，便于自动化消费。

### 处理结果
- `DiagnosticsSnapshot` 新增 `illegal_session_transitions / illegal_run_transitions / illegal_pipeline_transitions`，并完成计数与重置接入。
- 新增 CLI 命令：
  - `--seek-burst-serial-check`
  - `--paused-seek-serial-check`
  - `--close-reopen-serial-check`
- `--performance-log-check` 与 `--software-video-decode-check` 增加非法迁移计数字段输出。
- 新增分析文档：`docs/analysis/PLAYERCORE_DAY22_SERIAL_FAILSESSION_REGRESSION_CHECKS.md`。

### 本地验收结果
- Debug 构建通过，`0 warnings / 0 errors`。
- 新增 3 个检查命令在 `juren-30s.mp4` 上均返回 `PASS`。
...
- 本地验证：
  - Debug 构建通过，`0 warnings / 0 errors`。
  - `build\Debug\modern-video-player.exe --serial-failsession-regression-check .\juren-30s.mp4`：`PASS`（`pass_count=3/3`）。
...
- 新增 `--forced-failsession-check <media_file> [sample_ms]`，通过测试注入稳定覆盖 `FailSession` 路径，并输出机器可读 `key=value` + `result=PASS/FAIL`。
- `PlayerCore::decodeVideoFrame` 增加 `MVP_FORCE_FAIL_SESSION_ON_VIDEO_DECODE` 测试注入开关，用于强制进入 `FailureRecoveryPolicy::FailSession`。
- 修复 `FailSession` 从解码线程进入时的 codec 锁重入崩溃：
  - `video_codec_mutex_`、`audio_codec_mutex_` 调整为 `std::recursive_mutex`；
  - `decodeVideoFrame/decodeAudioFrame` 的 `lock_guard` 类型同步调整。
- 本地验证：
  - Debug 构建通过，`0 warnings / 0 errors`。
  - `build\Debug\modern-video-player.exe --forced-failsession-check .\juren-30s.mp4 2200`：`PASS`。
  - `build\Debug\modern-video-player.exe --serial-failsession-regression-check .\juren-30s.mp4`：`PASS`。
...
- `tools/run_all_checks.ps1` 新增参数：
  - `ForcedFailSessionFile`（默认复用 `ProbeFile`）
...
- gate 规则增强：
  - probe 失败直接退出；
  - forced-failsession 失败直接退出并跳过 format regression。
- 本地验证：
  ...
  - `probe/forced-failsession/regression` 退出码均为 `0`，脚本总退出码 `0`。

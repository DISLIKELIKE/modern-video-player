...
/// 按当前偏好重�?SDL renderer，并记录实际生效的驱动名称�?
...
/// 确保 renderer �?texture 与当前帧尺寸和格式匹配；必要时执行重建�?
...
/// 把待渲染帧上传到 SDL texture；支�?`YUV420P` �?`NV12` 直传�?
...
/// 根据鼠标位置更新 seek 预览；提交时只记录一次�?seek 请求�?

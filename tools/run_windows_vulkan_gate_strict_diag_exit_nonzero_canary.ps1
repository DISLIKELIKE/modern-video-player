param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-strict-diag-exit-nonzero-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-strict-diag-exit-nonzero-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_strict_diag_exit_nonzero_canary.cmd"
@'
@echo off
setlocal
...
} elseif (-not [string]::Equals($gateAvailabilityFailureDetail, "diag-exit-nonzero", [System.StringComparison]::Ordinal)) {
...
} elseif (-not [string]::Equals($gateDiagExitCode, "9", [System.StringComparison]::Ordinal)) {
...
$summary = [ordered]@{
    "windows-vulkan-strict-diag-exit-nonzero-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-strict-diag-exit-nonzero-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_result" = $gateResult
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_mode" = $gateMode
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_strict_mode_effective" = $gateStrictModeEffective
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_vulkan_availability_failure_detail" = $gateAvailabilityFailureDetail
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_playback_check_executed" = $gatePlaybackCheckExecuted
    "windows-vulkan-strict-diag-exit-nonzero-canary.gate_diag_exit_code" = $gateDiagExitCode
    "windows-vulkan-strict-diag-exit-nonzero-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-strict-diag-exit-nonzero-canary.result" = $result
}

param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-playback-semantic-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-playback-semantic-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_playback_semantic_canary.cmd"
@'
@echo off
setlocal
...
} elseif (-not [string]::Equals($gatePlaybackFailureDetail, "selected-renderer-not-vulkan", [System.StringComparison]::Ordinal)) {
    $validationFailureReason = "unexpected-playback-failure-detail"
} elseif (-not [string]::Equals($gatePlaybackSelectedRenderer, "D3D11", [System.StringComparison]::OrdinalIgnoreCase)) {
...
$summary = [ordered]@{
    "windows-vulkan-playback-semantic-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-playback-semantic-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-playback-semantic-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-playback-semantic-canary.gate_result" = $gateResult
    "windows-vulkan-playback-semantic-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-playback-semantic-canary.gate_playback_contract_valid" = $gatePlaybackContractValid
    "windows-vulkan-playback-semantic-canary.gate_playback_failure_detail" = $gatePlaybackFailureDetail
    "windows-vulkan-playback-semantic-canary.gate_playback_selected_renderer" = $gatePlaybackSelectedRenderer
    "windows-vulkan-playback-semantic-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-playback-semantic-canary.result" = $result
}

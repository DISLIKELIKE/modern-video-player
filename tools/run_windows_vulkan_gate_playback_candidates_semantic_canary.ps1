param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-playback-candidates-semantic-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-playback-candidates-semantic-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_playback_candidates_semantic_canary.cmd"
@'
@echo off
setlocal
...
if "%~1"=="--performance-log-check" (
  rem Contract-valid payload with semantic mismatch (candidates missing Vulkan).
  ...
  echo performance-log-check.startup_renderer_candidates=D3D11 ^> SoftwareSDL
...
} elseif (-not [string]::Equals($gatePlaybackFailureDetail, "candidates-missing-vulkan", [System.StringComparison]::Ordinal)) {
...
$summary = [ordered]@{
    "windows-vulkan-playback-candidates-semantic-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-playback-candidates-semantic-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-playback-candidates-semantic-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-playback-candidates-semantic-canary.gate_result" = $gateResult
    "windows-vulkan-playback-candidates-semantic-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-playback-candidates-semantic-canary.gate_playback_contract_valid" = $gatePlaybackContractValid
    "windows-vulkan-playback-candidates-semantic-canary.gate_playback_failure_detail" = $gatePlaybackFailureDetail
    "windows-vulkan-playback-candidates-semantic-canary.gate_playback_selected_renderer" = $gatePlaybackSelectedRenderer
    "windows-vulkan-playback-candidates-semantic-canary.gate_playback_renderer_backend" = $gatePlaybackRendererBackend
    "windows-vulkan-playback-candidates-semantic-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-playback-candidates-semantic-canary.result" = $result
}

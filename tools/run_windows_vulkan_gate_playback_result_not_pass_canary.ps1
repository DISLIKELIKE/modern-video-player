param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-playback-result-not-pass-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-playback-result-not-pass-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_playback_result_not_pass_canary.cmd"
@'
@echo off
setlocal
...
if "%~1"=="--performance-log-check" (
  rem Contract-valid payload where playback result is intentionally not PASS.
...
} elseif (-not [string]::Equals($gatePlaybackFailureDetail, "result-not-pass", [System.StringComparison]::Ordinal)) {
    $validationFailureReason = "unexpected-playback-failure-detail"
} elseif (-not [string]::Equals($gatePlaybackResult, "FAIL", [System.StringComparison]::OrdinalIgnoreCase)) {
...
$summary = [ordered]@{
    "windows-vulkan-playback-result-not-pass-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-playback-result-not-pass-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-playback-result-not-pass-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-playback-result-not-pass-canary.gate_result" = $gateResult
    "windows-vulkan-playback-result-not-pass-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-playback-result-not-pass-canary.gate_playback_contract_valid" = $gatePlaybackContractValid
    "windows-vulkan-playback-result-not-pass-canary.gate_playback_failure_detail" = $gatePlaybackFailureDetail
    "windows-vulkan-playback-result-not-pass-canary.gate_playback_result" = $gatePlaybackResult
    "windows-vulkan-playback-result-not-pass-canary.gate_playback_selected_renderer" = $gatePlaybackSelectedRenderer
    "windows-vulkan-playback-result-not-pass-canary.gate_playback_renderer_backend" = $gatePlaybackRendererBackend
    "windows-vulkan-playback-result-not-pass-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-playback-result-not-pass-canary.result" = $result
}

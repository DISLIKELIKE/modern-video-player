param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-auto-optional-no-probe-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-auto-optional-no-probe-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_auto_optional_no_probe_canary.cmd"
@'
@echo off
setlocal
...
if "%~1"=="--performance-log-check" (
  rem auto/no-probe path should skip playback check
...
try {
    ...
    Set-EnvVarForCanary -Name "MVP_WINDOWS_VULKAN_RUNTIME_PROBE_AVAILABLE" -Value "0" -Backup $envBackup
    Set-EnvVarForCanary -Name "MVP_WINDOWS_VULKAN_RUNTIME_PROBE_DETAIL" -Value "vulkaninfo-path-exit-1" -Backup $envBackup
...
} elseif (-not [string]::Equals($gateStrictAutoRuntimeProbeAny, "false", [System.StringComparison]::OrdinalIgnoreCase)) {
    $validationFailureReason = "unexpected-auto-runtime-probe-any-flag"
} elseif (-not [string]::Equals($gateStrictAutoRuntimeProbeSource, "none", [System.StringComparison]::Ordinal)) {
...
$summary = [ordered]@{
    "windows-vulkan-auto-optional-no-probe-canary.expected_gate_exit_code" = "0"
    "windows-vulkan-auto-optional-no-probe-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-auto-optional-no-probe-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-auto-optional-no-probe-canary.gate_result" = $gateResult
    "windows-vulkan-auto-optional-no-probe-canary.gate_mode" = $gateMode
    "windows-vulkan-auto-optional-no-probe-canary.gate_strict_mode_effective" = $gateStrictModeEffective
    "windows-vulkan-auto-optional-no-probe-canary.gate_skip_reason" = $gateSkipReason
    "windows-vulkan-auto-optional-no-probe-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-auto-optional-no-probe-canary.gate_vulkan_availability_failure_detail" = $gateAvailabilityFailureDetail
    "windows-vulkan-auto-optional-no-probe-canary.gate_playback_check_executed" = $gatePlaybackCheckExecuted
    "windows-vulkan-auto-optional-no-probe-canary.gate_strict_mode_auto_basis" = $gateStrictAutoBasis
    "windows-vulkan-auto-optional-no-probe-canary.gate_strict_mode_auto_prerequisites_met" = $gateStrictAutoPrerequisitesMet
    "windows-vulkan-auto-optional-no-probe-canary.gate_strict_mode_auto_runtime_probe_any_available" = $gateStrictAutoRuntimeProbeAny
    "windows-vulkan-auto-optional-no-probe-canary.gate_strict_mode_auto_runtime_probe_source" = $gateStrictAutoRuntimeProbeSource
    "windows-vulkan-auto-optional-no-probe-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-auto-optional-no-probe-canary.result" = $result
}

param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-strict-unavailable-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-strict-unavailable-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_strict_unavailable_canary.cmd"
@'
@echo off
setlocal
...
if "%~1"=="--performance-log-check" (
  rem strict-unavailable path should skip playback check
...
$summary = [ordered]@{
    "windows-vulkan-strict-unavailable-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-strict-unavailable-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-strict-unavailable-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-strict-unavailable-canary.gate_result" = $gateResult
    "windows-vulkan-strict-unavailable-canary.gate_mode" = $gateMode
    "windows-vulkan-strict-unavailable-canary.gate_strict_mode_effective" = $gateStrictModeEffective
    "windows-vulkan-strict-unavailable-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-strict-unavailable-canary.gate_vulkan_availability_failure_detail" = $gateAvailabilityFailureDetail
    "windows-vulkan-strict-unavailable-canary.gate_playback_check_executed" = $gatePlaybackCheckExecuted
    "windows-vulkan-strict-unavailable-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-strict-unavailable-canary.result" = $result
}

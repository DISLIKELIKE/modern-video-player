param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-playback-command-exit-nonzero-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-playback-command-exit-nonzero-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_playback_command_exit_nonzero_canary.cmd"
@'
@echo off
setlocal
...
if "%~1"=="--performance-log-check" (
  rem Contract-valid payload where command exits non-zero intentionally.
...
$gatePlaybackResult = Value-OrDefault -Map $gateSummary -Key "windows-vulkan-check.playback_result" -DefaultValue "missing"
...
} elseif (-not [string]::Equals($gatePlaybackFailureDetail, "command-exit-nonzero", [System.StringComparison]::Ordinal)) {
    $validationFailureReason = "unexpected-playback-failure-detail"
} elseif (-not [string]::Equals($gatePlaybackResult, "PASS", [System.StringComparison]::OrdinalIgnoreCase)) {
    $validationFailureReason = "unexpected-playback-result"
...
$summary = [ordered]@{
    "windows-vulkan-playback-command-exit-nonzero-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-playback-command-exit-nonzero-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_result" = $gateResult
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_playback_contract_valid" = $gatePlaybackContractValid
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_playback_failure_detail" = $gatePlaybackFailureDetail
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_playback_result" = $gatePlaybackResult
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_playback_selected_renderer" = $gatePlaybackSelectedRenderer
    "windows-vulkan-playback-command-exit-nonzero-canary.gate_playback_renderer_backend" = $gatePlaybackRendererBackend
    "windows-vulkan-playback-command-exit-nonzero-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-playback-command-exit-nonzero-canary.result" = $result
}

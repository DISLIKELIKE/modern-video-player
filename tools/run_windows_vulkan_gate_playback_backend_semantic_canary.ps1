param(
    ...
    [int]$SampleMs = 1200,
    [string]$SummaryOutputPath = "logs/windows-vulkan-gate-playback-backend-semantic-canary-summary.env",
    [string]$GateSummaryOutputPath = "logs/windows-vulkan-gate-playback-backend-semantic-canary-gate.env"
)
...
$mockExecutable = Join-Path $logsDir "mock_windows_vulkan_playback_backend_semantic_canary.cmd"
@'
@echo off
setlocal
...
if "%~1"=="--performance-log-check" (
  rem Contract-valid payload with semantic mismatch (backend is not Vulkan).
  echo performance-log-check.result=PASS
  echo performance-log-check.startup_selected_renderer=Vulkan
  echo performance-log-check.renderer_backend=OpenGL
...
} elseif (-not [string]::Equals($gatePlaybackFailureDetail, "renderer-backend-not-vulkan", [System.StringComparison]::Ordinal)) {
...
} elseif (-not [string]::Equals($gatePlaybackRendererBackend, "OpenGL", [System.StringComparison]::OrdinalIgnoreCase)) {
...
$summary = [ordered]@{
    "windows-vulkan-playback-backend-semantic-canary.expected_gate_exit_code" = "2"
    "windows-vulkan-playback-backend-semantic-canary.actual_gate_exit_code" = [string]$gateExitCode
    "windows-vulkan-playback-backend-semantic-canary.gate_summary_file_present" = if ($gateSummaryFilePresent) { "true" } else { "false" }
    "windows-vulkan-playback-backend-semantic-canary.gate_result" = $gateResult
    "windows-vulkan-playback-backend-semantic-canary.gate_failure_reason" = $gateFailureReason
    "windows-vulkan-playback-backend-semantic-canary.gate_playback_contract_valid" = $gatePlaybackContractValid
    "windows-vulkan-playback-backend-semantic-canary.gate_playback_failure_detail" = $gatePlaybackFailureDetail
    "windows-vulkan-playback-backend-semantic-canary.gate_playback_selected_renderer" = $gatePlaybackSelectedRenderer
    "windows-vulkan-playback-backend-semantic-canary.gate_playback_renderer_backend" = $gatePlaybackRendererBackend
    "windows-vulkan-playback-backend-semantic-canary.validation_failure_reason" = if ([string]::IsNullOrWhiteSpace($validationFailureReason)) { "none" } else { $validationFailureReason }
    "windows-vulkan-playback-backend-semantic-canary.result" = $result
}
